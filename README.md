...
- The `bundle gem` command will have initialized the project with its own
  ...
  git stuff
- Remove the `LICENSE.txt` file that was automatically generated by the
  `bundle gem` command. Run `rm LICENSE.txt` within the project.
- The `bundle gem` command will have already created a `README.md` file.
  Open it and delete its contents. Then add a main header with the project's
  name followed by a
